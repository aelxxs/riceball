...
export const abbreviateNumber = (value: number) => {
	...
	return Number.parseFloat(scaled.toFixed(1)) + suffix;
};
...
function relativeLuminance(color: RGBAColorObject): number {
	const a = (x: number): number => {
		...
		return normalizedX <= 0.03928 ? normalizedX / 12.92 : ((normalizedX + 0.055) / 1.055) ** 2.4;
	};
 ...
 */
export function formatContrastRatio(ratio: number, fractionDigits = 2): string {
