...
export default class implements Command {
	...
	public async chatInputRun({ guild }: Context, { amount }: Options) {
		...
		if (amount > economy.dailyRewardMax) {
			return "The minimum daily amount cannot be greater than the maximum daily amount.";
		}

		await this.db.setGuildSettings(guild.id, {
			economy: { dailyRewardMin: amount },
		});

		return `The minimum daily amount has been set to ${amount}.`;
	}
}
