...
export default class implements Command {
	...
	public async chatInputRun({ guild }: Context, { amount }: Options) {
		...
		if (amount < economy.dailyRewardMin) {
			return "The maximum daily amount cannot be less than the minimum daily amount.";
		}

		await this.db.setGuildSettings(guild.id, {
			economy: { dailyRewardMax: amount },
		});

		return `The maximum daily amount has been set to ${amount}.`;
	}
}
