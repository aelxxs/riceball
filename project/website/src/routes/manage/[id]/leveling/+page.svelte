<script lang="ts">
  ...
  import { LevelingUp, LevelUpRewards, TextLeveling } from "./sections";
  ...
  let embed = $state($form.notifyMessageEmbed);
            ...
            {#each palettes as palette}
              <div class="box palette-show cluster">
                <RadioGroup value="1" class="cluster space-2xs">
                  ...
                  >
                    {#each palette as color}
                      <div class="color" style:--color={hslaToStr(color)}></div>
                    {/each}
                  </div>
                </RadioGroup>
              </div>
