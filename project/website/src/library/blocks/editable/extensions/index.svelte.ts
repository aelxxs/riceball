		...
		return [
			Suggestion({
				...
				render: () => {
					...
					let componentProps: ComponentProps<typeof SuggestionList> | null = $state(null);
...
export const Tag = CustomSuggestion.extend({
	name: "tag-mention",
	addOptions() {
		return {
			type: "role" as const,
		...
		return [
			Suggestion({
				...
				render: () => {
					...
					let componentProps: ComponentProps<typeof EmojiList> | null = $state(null);
